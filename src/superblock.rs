...
pub fn emit_cfg_with_interrupts(
    ...
    writeln!(out, "; Generated by nes2gbc superblock emitter").unwrap();
            ...
            match ir::lower_instruction(*instruction) {
                Ok(ops) => {
                    ...
                    if in_trace_jump {
                        ...
                        writeln!(out, "    ; superblock: same-bank unique-entry JMP elided").unwrap();
                        continue;
                    }
...
mod tests {
    ...
    #[test]
    fn unique_entry_jump_becomes_superblock_fallthrough() {
        ...
        assert!(asm.contains("superblock: same-bank unique-entry JMP elided"));
    ...
    fn multiply_reached_target_is_not_internalized() {
        let mut prg = vec![0xEA; 0x8000];
        // BNE +3 reaches $8005; fallthrough JMP $8005 also reaches it.
