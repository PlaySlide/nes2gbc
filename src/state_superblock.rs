...
fn emit_stateful_branch(
    ...
    if state.a_live {
        writeln!(
            out,
            "    ld d, a ; preserve resident A across stateful branch"
        )
        .unwrap();
    }
    ...
    if state.a_dirty {
        ...
        writeln!(
            out,
            "    ldh [nes_a], a ; superblock side-exit materialize A"
        )
        .unwrap();
    ...
    if state.a_live {
        writeln!(
            out,
            "    ld a, d ; restore resident A on branch fallthrough"
        )
        .unwrap();
    }
...
pub fn emit_cfg_with_interrupts(
            ...
            match ir::lower_instruction(*instruction) {
                Ok(ops) => {
                    ...
                    if stateful_branch {
                        ...
                        let before = out.len();
                        let emitted =
                            emit_stateful_branch(&mut out, &ops, state, &mut stats, bank, &banks);
                        debug_assert!(emitted);
                        section_pc += approx_code_bytes(&out[before..]);
                        continue;
                    }
    ...
    println!(
        "superblock-state: A avoided {} reload(s), deferred {} store(s), eager materialized {}, side-exit materialized {}, seeds {}; avoided {} X + {} Y HRAM reload(s); deferred {} X + {} Y canonical store(s); eager materializations X={} Y={}; conditional side-exit materializations X={} Y={} across {} branch side exit(s); cached indexed uses X={} Y={}; seeds X={} Y={}; fast ops {} ({} compares, {} ADC/SBC, {} shifts/rotates, {} indexed RMW addr reuses); barriers {}; canonical adapters {}",
...
mod tests {
    ...
    fn conditional_side_exit_materializes_dirty_state_only_on_taken_path() {
        ...
        prg[0..10].copy_from_slice(&[0xA2, 0x04, 0xA9, 0x3F, 0xD0, 0x02, 0x85, 0x00, 0x60, 0x60]);
        let graph = cfg::discover(0, &prg, &[0x8000]).unwrap();
        let asm = emit_cfg_with_interrupts(
            &graph,
            EmitOptions {
