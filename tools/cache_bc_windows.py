...
CACHED_INDEX_RE = re.compile(r"ld a, ([bc])\s*;\s*BC window cache nes_([xy])$", re.IGNORECASE)
...
def has_long_lived_cache(body: list[str], reg: str) -> bool:
    for line in body:
        ...
        if not line_uses_reg(c, reg):
            continue
        cm = comment(line)
        if ("cached nes_" in cm or "seed nes_" in cm or "refresh nes_" in cm or
            "cached nes zp" in cm or "seed nes zp" in cm or "refresh nes zp" in cm or
            "cached 6502 a" in cm or "seed 6502 a" in cm or "refresh 6502 a" in cm or
            "cached x/y" in cm):
            return True
    return False
...
def state_stats(body: list[str], state: str) -> tuple[int, int, str | None]:
    loads = stores = 0; first: str | None = None
    for line in body:
        kind = state_access(line, state)
        if kind is None: continue
        if first is None: first = kind
        if kind == "load": loads += 1
        else: stores += 1
...
def zp_stats(body: list[str]) -> dict[int, tuple[int, int, bool, str | None]]:
    loads: dict[int,int] = {}; stores: dict[int,int] = {}; bad: set[int] = set(); first: dict[int,str] = {}
    for line in body:
        c = code(line)
        if m := ZP_LOAD_RE.fullmatch(c):
            addr = int(m.group(1),16); first.setdefault(addr,"load"); loads[addr] = loads.get(addr,0)+1; continue
        if m := ZP_STORE_RE.fullmatch(c):
            addr = int(m.group(1),16); first.setdefault(addr,"store")
            if m.group(2).lower() == "a": stores[addr] = stores.get(addr,0)+1
            else: bad.add(addr)
...
def optimize_window(lines:list[str], start:int, end:int, reg:str, stats:Stats)->None:
    if end <= start: return
    body = lines[start:end]; candidates:list[tuple[int,str,str|int]]=[]
    for state in ("nes_x","nes_y","nes_a"):
        loads,stores,first = state_stats(body,state); gain=hram_gain(loads,stores,first)
        if gain > 0: candidates.append((gain,"state",state))
    if not any(INDIRECT_MEM_RE.search(code(line)) for line in body):
        for addr,(loads,stores,bad,first) in zp_stats(body).items():
            if bad or loads == 0: continue
            gain=zp_gain(loads,stores,first)
            if gain > 0: candidates.append((gain,"zp",addr))
    if not candidates: return
    candidates.sort(key=lambda item:item[0], reverse=True); _gain,kind,key=candidates[0]; initialized=False
    for i in range(start,end):
        ind=indent_of(lines[i])
        if kind == "state":
            assert isinstance(key,str); access=state_access(lines[i],key)
            if access == "load":
                if initialized: lines[i]=f"{ind}ld a, {reg} ; BC window cache {key}\n"; stats.replaced_loads+=1
                else: lines[i]=lines[i]+f"{ind}ld {reg}, a ; seed BC window cache {key}\n"; initialized=True; stats.load_seeds+=1
            elif access == "store":
                was=initialized; lines[i]=lines[i]+f"{ind}ld {reg}, a ; refresh BC window cache {key}\n"; stats.mirrored_stores+=1; initialized=True
                if not was: stats.store_seeds+=1
        else:
            assert isinstance(key,int); c=code(lines[i]); lm=ZP_LOAD_RE.fullmatch(c)
            if lm and int(lm.group(1),16)==key:
                if initialized: lines[i]=f"{ind}ld a, {reg} ; BC window cache NES ZP ${key:02X}\n"; stats.replaced_loads+=1
                else: lines[i]=lines[i]+f"{ind}ld {reg}, a ; seed BC window cache NES ZP ${key:02X}\n"; initialized=True; stats.load_seeds+=1
                continue
            sm=ZP_STORE_RE.fullmatch(c)
            if sm and int(sm.group(1),16)==key and sm.group(2).lower()=="a":
                was=initialized; lines[i]=lines[i]+f"{ind}ld {reg}, a ; refresh BC window cache NES ZP ${key:02X}\n"; stats.mirrored_stores+=1; initialized=True
                if not was: stats.store_seeds+=1
    stats.windows+=1
    if kind=="state": stats.state_values+=1
    else: stats.zp_values+=1
...
def optimize_reg(lines:list[str], reg:str, stats:Stats)->None:
    for block in reversed(blocks(lines)):
        ...
        if has_long_lived_cache(body,reg): stats.reserved_blocks+=1; continue
        ws=block.start+1
        for i in range(block.start+1,block.end):
            if is_barrier(lines[i],reg): optimize_window(lines,ws,i,reg,stats); ws=i+1
        optimize_window(lines,ws,block.end,reg,stats)
...
def fold_direct_cached_indexes(lines:list[str])->tuple[int,int,int]:
    ...
    for i in range(len(lines)):
        ...
        if kind is None:
            p=prev_code_index(lines,i-1,max(0,i-4)); page_marker=any("dead host-flag scaffold removed: page-aligned index" in lines[k] for k in range(j+1,min(len(lines),j+6)))
            if p is not None and PAGE_BASE_RE.fullmatch(code(lines[p])) and page_marker: kind="page"
        if kind is None: continue
...
def optimize(lines:list[str])->tuple[Stats,int,int,int]:
    stats=Stats(); optimize_reg(lines,"b",stats); normalize(lines); optimize_reg(lines,"c",stats); normalize(lines); prg,page,zp0=fold_direct_cached_indexes(lines); return stats,prg,page,zp0

def main()->int:
    p=argparse.ArgumentParser(); p.add_argument("asm",type=Path); args=p.parse_args(); lines=args.asm.read_text(encoding="utf-8").splitlines(keepends=True); s,prg,page,zp0=optimize(lines); args.asm.write_text("".join(lines),encoding="utf-8")
    print(f"bc-window: cached {s.state_values} CPU-state + {s.zp_values} ZP value(s) across {s.windows} straight-line window(s), replaced {s.replaced_loads} loads, mirrored {s.mirrored_stores} stores; seeded {s.load_seeds} on first load / {s.store_seeds} from prior store; reserved {s.reserved_blocks} long-lived-cache register/block pair(s); fed {prg}/{page}/{zp0} cached X/Y index(es) directly into PRG/page/ZP00 addresses")
    return 0
