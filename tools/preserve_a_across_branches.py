...
* requires a visible canonical ``ldh [nes_a], a`` immediately before the branch
  flag test, with only A-preserving stores/copies in between;
* saves clean A in E, evaluates the existing shadow test, and restores A before
  the host conditional JR (LD does not change LR35902 flags);
...
def optimize(lines: list[str]) -> tuple[int, int, int]:
    ...
    # (save_before_line, restore_after_line, transfer_line, target_label, opcode)
    ...
    for block in blocks:
        ...
        for p in range(0, max(0, len(active) - 3)):
            ...
            load_c = code(lines[load_i])
            test_c = code(lines[test_i])
            if not is_shadow_test_pair(load_c, test_c):
                continue
            if not COND_SKIP_RE.fullmatch(code(lines[skip_i])):
                continue
            ...
            # First experiment: hot loop-shaped taken edges only.
            ...
            edits.append((load_i, test_i, transfer_i, label, tm.group(1)))
            target_addrs.add(target)

    if not edits:
        return 0, 0, 0

    # Rewrite source lines in place first; insertions are collected separately.
    insert_before: dict[int, list[str]] = {}
    insert_after: dict[int, list[str]] = {}
    for load_i, test_i, transfer_i, label, opcode in edits:
        ...
        lines[transfer_i] = f"{tind}{opcode} {label}\n"
    ...
    return len(edits), existing_used, created
...
def main() -> int:
    ...
    edges, reused, created = optimize(lines)
