...
.fit_chr_upload:
    ...
    ; LCD is already off here: blank atlas tile 0 once, reset cache metadata only
    ; (never fill_zero BG maps mid-game — that caused flash/meltdown).
    call nes_video_fit_upload_sprite_chr_raw
    call nes_video_fit_blank_tile0
...
nes_video_sync_nametable_write:
    ...
    ; Fit-screen: pack 2x2 NES tiles into one GBC 8x8 metatile (no screen-door).
    ld a, [nes_fit_screen]
    and a
    ld a, c
...
nes_video_update_horizontal_stitch:
    ; Fit-screen uses a half-res packed map; stitching would fight that layout.
    ld a, [nes_fit_screen]
    and a
    jr nz, .disable
...
;; ---------------------------------------------------------------------------
; Fit-screen 2x2 nametable metatile packing (blit-on-miss)
; ---------------------------------------------------------------------------
...
; On cache miss: compose one 8x8 in WRAM from ROM GBC CHR, then blit 16 bytes
; into one atlas slot with wait_vram (LCD stays on). Never clear BG maps here.
...
; Blank reserved atlas tile 0 in VRAM bank 0. Caller must own LCD-off.
nes_video_fit_blank_tile0:
    xor a
    ldh [rVBK], a
    ld hl, $8000
    ld b, 16
...
; Rare PPUCTRL.3 change: refresh sprite CHR in bank 1. Prefer LCD-off during
; VBlank for the 4 KiB copy (wait_vram byte loop would stall for many frames).
; Does not clear BG maps or reset the metatile cache.
...
; Metadata-only cache reset. Never fill_zero $9800/$9C00.
...
; HL = aligned metatile top-left in bank-1 NT RAM.
...
.row2_ok:
    ...
    push hl
    call nes_video_fit_cache_lookup
    ld [nes_fit_mt_tmp_l], a
    pop hl
...
.alloc_done:
    ...
    ; Save/restore IE — never blind EI after DI.
    ldh a, [rIE]
    push af
    xor a
    ldh [rIE], a
...
; A = atlas slot. Blit 16 WRAM compose bytes into VRAM bank 0 with wait_vram.
...
; Refresh CGB attrs for the 2x2 metatiles covered by NES attr byte at HL.
; Does not recompose tiles — only palette bits.
...
.meta_loop:
    ...
    ; TL NES address for palette decode
    ld a, [nes_fit_mt_mx]
    add a
    ld e, a
    ld a, [nes_fit_mt_my]
